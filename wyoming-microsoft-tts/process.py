...
class MicrosoftProcess:
    """Info for a running Microsoft process (one voice)."""
...
class MicrosoftProcessManager:
    """Manager of running Microsoft processes."""

    def __init__(self, args: argparse.Namespace, voices_info: Dict[str, Any]):
        self.voices_info = voices_info
        self.args = args
        self.processes: Dict[str, MicrosoftProcess] = {}
        self.processes_lock = asyncio.Lock()

    async def get_process(self, voice_name: Optional[str] = None) -> MicrosoftProcess:
        """Get a running Microsoft process or start a new one if necessary."""
        ...
        microsoft_proc = self.processes.get(voice_name)
        if (microsoft_proc is None) or (microsoft_proc.proc.returncode is not None):
            # Remove if stopped
            self.processes.pop(voice_name, None)

            # Start new Microsoft process
            if self.args.max_microsoft_procs > 0:
                # Restrict number of running processes
                while len(self.processes) >= self.args.max_microsoft_procs:
                    ...
                    if lru_proc.proc.returncode is None:
                        try:
                            ...
                            _LOGGER.exception("Unexpected error stopping microsoft process")
            ...
            microsoft_proc = MicrosoftProcess(
                name=voice_name,
            ...
            self.processes[voice_name] = microsoft_proc

        # Update used
        microsoft_proc.last_used = time.monotonic_ns()

        return microsoft_proc